...
const { VERSION_AMMV2, DEPLOY_KEY  } = process.env
...
execSync(`graph deploy ${TARGET_CHAIN.ammv2Name} --version-label ${VERSION_AMMV2} --node https://subgraphs.alchemy.com/api/subgraphs/deploy --deploy-key ${DEPLOY_KEY} --ipfs https://ipfs.satsuma.xyz`, { stdio: 'inherit' })